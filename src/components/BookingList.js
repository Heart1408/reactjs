...
import { useGetListProduct, useGetCategories } from "../hooks/category";
import { useAddDishToMenu } from "../hooks/customer/booking";
import { useAddDishToBooking } from "../hooks/schedule";
import { UndoOutlined, ArrowLeftOutlined } from "@ant-design/icons";
import { Select, Input, Button, List, Divider, message, Checkbox } from "antd";
...
function BookingList(props) {
  const { onShowConfirmCustomerModel, confirmCus } = props;
  ...
  const { data: dataCategoryResponse } = useGetCategories();
  ...
  const addDishToBooking = useAddDishToBooking(handleResponse);
  ...
  const addToBooking = () => {
    addDishToBooking.mutate({
      bookingId: bookingId,
      listOrdered: serveredList,
    })
  }
    ...
    <div className="menu-page">
      {bookingId !== null && (
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          ...
          <Button onClick={addToBooking} type="primary" style={{marginRight: "6%"}}>Thêm</Button>
        </div>
      )}
      <div>
        <Slider>
